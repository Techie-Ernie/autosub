import argparse
import os
from utils import format_time, transcribe_audio, add_subtitles
...
def main():
    ...
    if not args.video_path:
        print('Error: No video path provided. Please provide the path to the video file.')
    else:
        video_clip = VideoFileClip(args.video_path)
        audio_clip = video_clip.audio
        audio_clip.write_audiofile(os.path.splitext(args.video_path)[0]+'.mp3')
        srt_file = transcribe_audio(input_file=os.path.splitext(args.video_path)[0]+'.mp3', words=args.words, gpu=args.gpu)

        if args.srt_only:
            print(f'srt file found at {srt_file}')
        else:
            add_subtitles(os.path.splitext(args.video_path)[0]+'.mp3', args.video_path, srt_file, os.path.splitext(args.video_path)[0]+'_final.mp4')
